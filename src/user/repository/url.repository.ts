...
export class UrlRepository {
  ...
  async findUserLinks(userId: Types.ObjectId, skip: number, limit: number) {
  ...
  async countUserLinks(userId: Types.ObjectId) {
    return await this._urlModel.countDocuments({ userId: userId });
  }

  async findOriginalUrl(id: string) {
    return await this._urlModel.findOne({ shortenedLink: id });
  }
}
